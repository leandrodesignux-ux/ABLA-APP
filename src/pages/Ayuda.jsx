...
import AblaScene from '../components/AblaScene.jsx'
...
const intentions = [
  { title: 'Necesito hablar', description: 'Conversa de forma confidencial.', to: '/chat/anonimo', sceneType: 'chat' },
  { title: 'Necesito orientación', description: 'Encuentra consejos para lo que estás viviendo.', to: '/ayuda/consejos', sceneType: 'advice' },
  { title: 'Quiero pedir una cita', description: 'Agenda apoyo con un profesional.', to: '/ayuda/cita', sceneType: 'appointment' },
]

function IntentionCard({ title, description, to, sceneType }) {
  const navigate = useNavigate()
  const reducedMotion = useReducedMotion()
  return <motion.button type="button" initial="idle" animate="idle" whileHover="hover" whileTap="tap" variants={{ idle: {}, hover: motionIfAllowed(reducedMotion, { y: -3 }), tap: motionIfAllowed(reducedMotion, ablaMotion.press) }} onClick={() => navigate(to)} className="flex min-h-44 items-center gap-4 rounded-abla-card bg-white p-5 text-left shadow-abla-card transition-shadow hover:shadow-abla-float md:flex-col md:items-start"><motion.div variants={{ idle: { scale: 1 }, hover: { scale: reducedMotion ? 1 : 1.015 }, tap: { scale: .99 } }} className="h-28 w-28 shrink-0 overflow-hidden rounded-[44%_56%_46%_54%/56%_42%_58%_44%] bg-abla-green-soft md:h-36 md:w-full"><AblaScene type={sceneType} /></motion.div><div><h2 className="text-lg font-black text-abla-blue">{title}</h2><p className="mt-1 text-sm leading-5 text-slate-500">{description}</p></div></motion.button>
}
    ...
    <PageTransition>
      <div className="min-h-dvh bg-abla-bg pb-24 md:pb-12">
        ...
        <main className="mx-auto w-full max-w-6xl px-4 py-6 md:px-6 md:py-10 lg:px-8">
          <section className="grid items-center gap-6 overflow-hidden rounded-abla-panel bg-abla-blue p-6 text-white md:grid-cols-[1fr_240px] md:p-9"><div><p className="text-sm font-bold uppercase tracking-[.16em] text-white/65">Estamos contigo</p><h1 className="mt-2 text-3xl font-black leading-tight md:text-4xl">¿Qué necesitas ahora?</h1><p className="mt-3 max-w-xl text-sm leading-6 text-white/75 md:text-base">Puedes hablar, buscar orientación o pedir apoyo profesional. Elige por dónde quieres empezar.</p></div><div className="relative mx-auto"><span className="absolute -right-3 top-3 h-16 w-24 rounded-[24px_24px_24px_7px] bg-white/10" /><AblaCharacter emotion="safe" shape="arch" pose="open" gaze="left" interaction="supportive" size="xl" animate="float" decoration blink label="Personaje de apoyo ABLA" /></div></section>
