...
export default function AblaCompanion({ personality = 'friendly', mood, pose, gaze, accessory = 'none', size = 'md', interactive = false, animate = true, label, className = '' }) {
  ...
  return <motion.svg viewBox="0 0 120 120" width={dimension} height={dimension} className={className} role={label ? 'img' : undefined} aria-label={label} aria-hidden={label ? undefined : true} {...states}>
    <motion.g animate={!reducedMotion && animate && personality === 'calm' ? { scaleX: [1, .995, 1], scaleY: [1, 1.015, 1] } : undefined} transition={{ duration: companionTokens.timing.calm, repeat: Infinity, ease: 'easeInOut' }} style={{ transformOrigin: '60px 64px' }}>
      <CompanionArms pose={currentPose} variants={armVariants} />
      <CompanionBody variants={bodyVariants} />
      <g color={companionTokens.color.ink}><CompanionEyes expression={face.eyes} gaze={currentGaze} blink={!reducedMotion && animate && large} /><CompanionMouth expression={face.mouth} /></g>
      <CompanionAccessory type={accessory} />
      {large && <g aria-hidden="true"><circle cx="12" cy="29" r="3" fill={companionTokens.color.blueSoft} /><path d="M104 9l3 6 6 3-6 3-3 6-3-6-6-3 6-3Z" fill="#9FC8F5" opacity=".8" /></g>}
    </motion.g>
  </motion.svg>
}
